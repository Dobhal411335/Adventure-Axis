...
const CreateBrandName = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!formData.frontImg?.url || !formData.frontImg?.key) return toast.error("Please upload a image");
        try {
            ...
            if (response.ok) {
                toast.success(`Brand ${editBanner ? "updated" : "added"} successfully`);
                setEditBanner(null);

                // Refresh banner list
                const updatedBanners = await fetch("/api/addBrand").then((res) => res.json());
    ...
    return (
        <div className="max-w-5xl mx-auto py-10 w-full">
            <h2 className="text-2xl font-bold mb-6">{editBanner ? "Edit Brand" : "Add New Brand"}</h2>
            <form onSubmit={handleSubmit} className="bg-white shadow-lg rounded-lg p-6 space-y-4">
                {/* Banner Image Upload */}
                <div className="mb-4">
                    <Label className="block mb-2 font-bold">Brand Image</Label>
                    <input
                        type="file"
                        accept="image/*"
                ...
                <div className="flex gap-2">
                    <Button type="submit" className="bg-blue-600 hover:bg-blue-500">
                        {editBanner ? "Update Brand" : "Add Brand"}
                    </Button>
                    {editBanner && (
                        <Button
                            type="button"
            ...
            <Table>
                ...
                <TableBody>
                    {banners.length > 0 ? (
                        ...
                        <TableRow>
                            <TableCell colSpan="5" className="text-center py-4">No Brand found</TableCell>
                        </TableRow>
                    )}
                </TableBody>
            </Table>
